  ...
  createLike(currentLikeBox) {
    jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
      ...
      success: response => {
        ...
        currentLikeBox.attr("data-exists", "yes"); // Increment count

        let likeCount = parseInt(currentLikeBox.find(".like-count").html(), 10);
        likeCount++; // Output new number in html

        currentLikeBox.find(".like-count").html(likeCount); // Update the data-like
  ...
  deleteLike(currentLikeBox) {
    jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
      ...
      success: response => {
        ...
        currentLikeBox.attr("data-exists", "no"); // Decrease number
...
class MyNotes {
  ...
  makeNoteEditable(thisNote) {
    ...
    thisNote.find(".edit-note").html('<i class="fa fa-times" aria-hidden="true"></i> Cancel'); // Remove read only attribute

    thisNote.find(".note-title-field, .note-body-field").removeAttr("readonly").addClass("note-active-field"); // Display the update button

    thisNote.find(".update-note").addClass("update-note--visible"); // Add data state
  ...
  makeNoteReadOnly(thisNote) {
    ...
    thisNote.find(".edit-note").html('<i class="fa fa-pencil" aria-hidden="true"></i> Edit'); // Add read only attribute

    thisNote.find(".note-title-field, .note-body-field").attr("readonly", "readonly").removeClass("note-active-field"); // Hide the update button

    thisNote.find(".update-note").removeClass("update-note--visible"); // Add data state
  ...
  updateNote(e) {
    ...
    const thisNote = jquery__WEBPACK_IMPORTED_MODULE_0___default()(e.target).parents("li"); // Create new post object
...
class Search {
  ...
  typingLogic() {
    ...
    if (this.searchField.val() != this.previousValue) {
      ...
      clearTimeout(this.typingTimer); // Check for empty

      if (this.searchField.val()) {
        // Only if search field is not empty run this
        // Only add spinner code if not already there
        if (!this.isSpinnerVisible) {
          this.resultsDiv.html('<div class="spinner-loader"></div>'); // Manage state for spinner
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default().getJSON(universityData.root_url + "/wp-json/university/v1/search?term=" + this.searchField.val(), results => {
      ...
      <div class="row">
        <div class="one-third">
          ...
          ${results.generalInfo.length ? '<ul class="link-list min-list" >' : `<p>No general information matches that search.</p><p><a href=${universityData.root_url}/posts>View all posts.</a></p></p>`}
        ...
        <div class="one-third">
          ...
          ${results.skills.length ? '<ul class="link-list min-list" >' : `<p>No skills match that search. <a href=${universityData.root_url}/skills>View all skills.</a></p>`}
        ...
        <div class="one-third">
          ...
          ${results.clients.length ? '<ul class="professor-cards" >' : `<p>No clients match that search. <a href=${universityData.root_url}/clients>View all clients.</a></p>`}
  ...
  openOverlay() {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default()("body").addClass("body-no-scroll"); // Reset search field when the overlay is opened again

    this.searchField.val(""); // Wait for element to load properly
